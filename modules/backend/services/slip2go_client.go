...
// Slip2GoClient calls Slip2Go's bank-slip verification API
...
// NewSlip2GoClient returns nil if secret is empty — callers must treat a nil
// client as "Slip2Go not configured" and skip verification, same pattern as
// R2Presigner.
func NewSlip2GoClient(apiURL, secret string) *Slip2GoClient {
	if secret == "" {
		return nil
	}
...
// SlipInfo is the subset of Slip2Go's slip data we act on. Matching against a
// teacher's wallet is done in our own code (see MatchesWallet) rather than
// via Slip2Go's checkCondition — checkCondition's accountNumber match failed
// in practice against a PromptPay-proxy receiver (bank.account is null for
// those; only the masked proxy number is available), and Slip2Go truncates
// receiver names, so exact matching there doesn't work for our case.
...
// calls (see PIPELINE.md Stage 11):
...
// is sent; matching against a specific teacher's wallet is done by the
// caller via MatchesWallet.
func (c *Slip2GoClient) GetSlipInfo(ctx context.Context, qrCode string) (*SlipInfo, error) {
...
// Slip2Go's name is Thai-title-prefixed and truncated, and teachers can be
...
// the caller separately checks the transferred amount against the course
// price.
